...
使用 WordPress 蒐集文章者需要的小工具，上網看到文章想收錄到自己的 WordPress 時使用。
...
### Plurk 串備份
1. 開啟要備份的 Plurk 單噗頁面（`plurk.com/p/xxxxxx`）
2. 點 ✂ 圖示，popup 自動進入 **Plurk 串備份模式**
3. 點「**展開全部回應**」載入所有留言（長串需等候）
4. 勾選篩選選項（備份影像、備份連結、排除純表情回應）
5. 點「**全部備份**」直接匯入 WordPress 草稿

### Threads 貼文擷取
1. 開啟單篇 Threads 貼文（`threads.com/@user/post/<id>`）
2. 點 ✂ 圖示，popup 自動進入 **Threads 貼文模式**
3. **手動捲動頁面瀏覽完整討論串**（重要：Threads 採虛擬列表，捲出視窗的貼文會被卸載；本擴充功能會在背景持續累積你看過的每一則）
   - 或點「**載入全部回覆**」自動以遞增式捲動載入
4. 點「**✂ 匯入為草稿**」匯入到 WordPress
5. 主貼文 + 作者續串 + 所有留言依 DOM 順序匯入；作者本人回覆自動加上「【作者】」前綴
...
- `udn.com` — 聯合新聞網
- `facebook.com` — 臉書（預設保留留言；需點入單篇文章，從時間處按滑鼠中鍵另開分頁）
- `plurk.com/p/*` — Plurk 單噗頁面（備份模式）
- `threads.com` / `threads.net` — Threads 單篇貼文（持續累積式擷取，可解析作者續串與所有留言）
